...
oil_palm_2019 = oil_palm_2019.to_crs(gdf_boundary_LAEA_Borneo.crs)

oil_palm_2019_borneo = oil_palm_2019.clip(gdf_boundary_LAEA_Borneo)
...
tif_files = find_tif_files_with_polygon(root_folder, gdf_boundary, keyword, minimum_area)
...
for tif_path in tif_files:
    ...
    clip_tif_to_bbox(input_file, gdf_boundary, clipped_output_file)
    ...
    change_values_outside_mask(input_file, output_file, gdf_boundary, new_value=0)
    ...
    output_directory = "../../07_data_prepared/crop_type/crops_2000/utm"
    output_file = os.path.join(output_directory, "utm_" + file_name)
    reproject_raster("EPSG:23846", input_file, output_file)
```
...
for tif_path in tif_files:
    ...
    # Reproject the masked TIFF
    input_file = output_file
    output_directory = "../../07_data_prepared/crop_type/crops_2010/utm"
    output_file = os.path.join(output_directory, "utm_" + file_name)
    reproject_raster("EPSG:23846", input_file, output_file)
```
...
for tif_path in tif_files:
    ...
    # Reproject the masked TIFF
    input_file = output_file
    output_directory = "../../07_data_prepared/crop_type/crops_2015/utm"
    output_file = os.path.join(output_directory, "utm_" + file_name)
    reproject_raster("EPSG:23846", input_file, output_file)
```
...
# Define the GeoDataFrame boundary (gdf_boundary) and EPSG code (epsg)
epsg = 

# Iterate over the TIFF files in the list
