...
```{python}
WGS84 = "EPSG:4326"
gdf_boundary_WGS84 = ox.geocode_to_gdf("Malaysia").dropna(axis=1, how="any").to_crs(WGS84)

# Export the GeoDataFrame as a shapefile
gdf_boundary_WGS84.to_file("../../07_data_prepared/boundary/malaysia_boundary_WGS84.shp")

input_shp = "../../07_data_prepared/boundary/malaysia_boundary_WGS84.shp"
output_shp = "../../07_data_prepared/boundary/malaysia_boundary_LAEA.shp"
...
```{python}
gdf = gpd.read_file("../../07_data_prepared/boundary/borneo_boundary_LAEA_Borneo.shp")
area_in_square_meters = gdf['geometry'][0].area

# Convert the area to square kilometers
area_in_square_km = area_in_square_meters / 1e6
print(f"Area in Square Kilometers: {area_in_square_km} km²")
```
...
## everything detected before 2000 is changed to value 2000, all other data is changed to mach the gfw structure
```{python}
...
reclassify_values = {2000: list(range(1, 21)),
                     ...
                     16: 36,
                     17: 37}
input_tif = '../../07_data_prepared/oil_palm/detection_year.tif'
output_tif = '../../07_data_prepared/oil_palm/detection_year_2000_2017.tif'
...
## everything detected before 2001 is changed to nodata (0)
```{python}


reclassify_values = {0: list(range(1, 21)),
                     1: 21,
                     2: 22,
                     3: 23,
                     4: 24,
                     ...
                     17: 37}
input_tif = '../../07_data_prepared/oil_palm/detection_year.tif'
output_tif = '../../07_data_prepared/oil_palm/detection_year_2001_2017.tif'
...
# reproject to LAEA Borneo
reproject_raster_to_wktcrs('../../07_data_prepared/oil_palm/detection_year_2001_2017.tif', '../../07_data_prepared/oil_palm/LAEA_detection_year_2001_2017.tif')
...
# reproject to epsg:23846
reproject_raster_to_wktcrs('../../07_data_prepared/deforestation/gfw_gain_2000_2012_borneo.tif', '../../07_data_prepared/deforestation/LAEA_gfw_gain_2000_2012_borneo.tif')
...
## 2000
```{python}
root_folder = '../../06_data/vegetation/crops_2000'
keyword = 'A.tif' 
minimum_area = 1
...
for tif_path in tif_files:
    ...
    replace_nan_with_zero(clipped_output_file, clipped_output_file)

    # change nodata value to 0 (see metadata)
    change_sub_zero_to_zero(clipped_output_file, clipped_output_file)
...
## 2005
```{python}
root_folder = '../../06_data/vegetation/crops_2005'
keyword = '_A_TA_' 
minimum_area = 1
...
for tif_path in tif_files:
    ...
    # Clip the TIFF to the boundary
    input_file = tif_path
    output_directory = "../../07_data_prepared/crop_type/crops_2005/original_crs"
    clipped_output_file = os.path.join(output_directory, file_name.replace(".tif", "_clipped.tif"))
    clip_tif_to_bbox(input_file, gdf_boundary_WGS84, clipped_output_file)
    ...
    # Reproject the masked TIFF
    input_file = output_file
    output_directory = "../../07_data_prepared/crop_type/crops_2005/LAEA"
    output_file = os.path.join(output_directory, "LAEA_" + file_name)
    reproject_raster_to_wktcrs(input_file, output_file)
```
...
## 2010
```{python}
root_folder = '../../06_data/vegetation/crops_2010/spam2010v2r0_global_phys_area.geotiff'
keyword = 'H.tif' 
minimum_area = 1
...
for tif_path in tif_files:
    ...
    # convert all vlaues to integers
    round_and_export_geotiff(clipped_output_file, clipped_output_file)
