...
labels = ['Primary Forest (2001)', '>= 75% tree cover', '< 75% tree cover']
...
# Use seaborn's barplot function with the specified colors
sns.barplot(x='Year', y='total', data=df, label='New palm oil plantations after deforestation', alpha=1, ax=ax, color=colors[0], zorder=2)
ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: '{:,.1f}'.format(x / 1e6)))

# Set title and labels
ax.set_xlabel('Year')
ax.set_ylabel('new oil palm after deforeastation (Mha)')
...
# Add a custom legend entry for the total sum
custom_legend_entry = f'Total new oil palm plantation after deforestation: {round(total_sum/1000000, 2)} Mha'

# Add text at an arbitrary location of the Axes
ax.text(x=0, y=y_max * 1.05, s=custom_legend_entry, fontsize=12)
...
fig, ax = plt.subplots(figsize=(7, 5))  # Adjust the width and height as needed
...
# Add the sum of the 'total' column to the plot
ax.text(df['Year'].min(), y_max * 1.2, f'Total new palm oil plantations in RSPO certified concessions: {round(total_sum/1000000, 2)} mio Mha', ha='left', va='center')
...
# Add the sum of the 'total' column to the plot
ax.text(df['Year'].min(), y_max * 1.2, f'Total new palm oil plantations in RSPO uncertified concessions: {round(total_sum/1000000, 2)} mio Mha', ha='left', va='center')
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.25)  # adjust the bottom space
...
for i, (title, width_file) in enumerate(zip(titles, width_files)):
    ...
    ax.set_title(title, fontsize=14, fontweight='bold')
    ax.set_xlabel('area of primarycovered by buffer (Mha)')
